...
| API-07 | API | FR-09, AC-10/11 | Staff queue search/filter/sort/pagination/defaults, Requested/IT Priority filters and sorting, and safe failures — `server/tests/lab-03/staff-queue.api.test.ts` | Passed: 5 focused tests; full server regression awaits local Lab 3 password variables |
...
| UI-04 | Component | FR-09/17, AC-10/11/21 | Queue controls, Requested/IT Priority filters and sorting, states, role visibility, cards/table, and pagination — `client/tests/lab-03/StaffTicketQueue.test.tsx` | Passed: 4 focused tests; full client suite 9 files/40 tests |
...
| REG-02 | Regression | AC-06/07/08/21 | Complete prior client suite after selector-to-auth migration — `client/tests/lab-01/**`, `client/tests/lab-02/**`, `client/tests/lab-03/**` | Passed: 9 files, 40 tests |
...
- [x] Queue API focused tests cover search, category/system, Requested/IT Priority, status, owner/me/unassigned, sorting, stable defaults, page sizes, inactive/ineligible references, and exact response metadata (5 tests).
- [x] Queue UI focused tests cover controls, URL query state, table/cards, loading data, empty/no-results distinction, safe Retry, and forbidden state (4 tests).
...
- [x] Server build, client build, and complete client suite (9 files/40 tests) pass for the current branch.
